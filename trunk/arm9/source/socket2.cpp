...
CSocket2::CSocket2(bool aBlock): iSocket(-1)
{
...
CSocket2::CSocket2(int aSocket,bool aBlock): iSocket(aSocket)
{
  SetBlock(aBlock);
}
...
CSocket2* CSocket2::Accept(void)
{
  ...
  CSocket2* result=new CSocket2(newSocket,iBlock);
  return result;
}

int CSocket2::Receive(char* aBuffer,int aSize)
{
  int lenb;
  while(true)
  {
...
int CSocket2::Send(const char* aBuffer,int aSize)
{
  int total=0;
  do
  {
...
void CSocket2::SetBlock(bool aBlock)
{
  iBlock=aBlock;
  unsigned long arg=iBlock?1:0;
